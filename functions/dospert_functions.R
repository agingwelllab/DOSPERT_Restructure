#create function to restructure df for analysis
dclean <- function(draw_df){
  ...
  drt_long <- select(drt_long, -risk_type) #remove 'risk_type' column
  #bind risk type dfs together to create one long df for all risk types
  dtotal_long <- cbind(drt_long, drp_long, drb_long)
  dtotal_clean <- dtotal_long %>% #remove redundant 'variables' columns
    select(-variables)
  dtotal_clean <- arrange(dtotal_clean, record_id, domain) #arrange clean df by record id and domain
}
...
dsum <- function(dtotal_clean) {
  ...
  dtotal_sum <- aggregate(cbind(RT,RP,RB) ~ record_id + domain, data = dtotal_clean, FUN = sum)
